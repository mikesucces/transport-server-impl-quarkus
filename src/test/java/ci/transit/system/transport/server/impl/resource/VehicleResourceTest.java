...
class VehicleResourceTest {
    ...
    void payrollSumsFixedSalaries() {
        given().when().get("/api/drivers/payroll")
            .then().statusCode(200)
    ...
    void controllerCannotReadPayroll() {
        given().when().get("/api/drivers/payroll").then().statusCode(403);
    }

    @Test
    @TestSecurity(user = "owner", roles = "OWNER")
    void fleetAlertsAggregateEverything() {
        given().when().get("/api/fleet/alerts")
            .then().statusCode(200)
